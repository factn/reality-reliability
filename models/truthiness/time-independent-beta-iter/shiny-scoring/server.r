...
shinyServer(function(input, output, session) {
    ...
    prev_res <- reactive({
        req(input$iter)
        if (input$iter > 0) {
            mod <- models[which(iter_n == input$iter)-1, file]
            prev <- get_mod_res(mod)
            return(prev)
        } else {
            prev <- NULL
            return(prev)
    ...
    sel_res <- reactive({
        req(input$iter)
        mod <- models[which(iter_n == input$iter), file]
        sel <- get_mod_res(mod)
        return(sel)
    })
    ...
    three_versions <- reactive({
        ...
        foresight <- wasserstein1d(f_truth[, value], ans$answer)
        ...
        dat <- list(p_truth=p_truth, p_prec=p_prec, c_truth=c_truth, c_prec=c_prec,
                    ...
                    innovation=innovation, foresight=foresight, shift=shift, score=score)
    ...
    output$score_txt2 <- reactive({
        req(three_versions())
        dat <- three_versions()
        sprintf('<div style="text-align: center;">Innovation: %0.4f<br>Foresight: %0.4f<br>Shift: %0.4f</div>',
                dat$innovation, dat$foresight, dat$shift)
    })
