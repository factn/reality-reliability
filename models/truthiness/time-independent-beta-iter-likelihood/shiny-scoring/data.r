...
ll_curr <- allresults[vartype == 'll_next_response']
ll_final <- allresults[vartype == 'll_response']

ll_curr[ll_final, ll_final := i.value, on = c('ll_iter', 'sample', 'ch', 'ch_sample')]
likelihoods <- ll_curr[, .(ll_iter, sample, ch, ch_sample, ll_curr = value, ll_final)]
...
save(allresults, simdata0, models, truth_final, prec_final, truth_prior, prec_prior, likelihoods,
     real_truth, real_prec, file = 'data/data.rdata', compress=F)
